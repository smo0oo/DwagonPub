...
public class PlayerMovement : MonoBehaviour, IMovementHandler
{
    ...
    void Start() { wasdMoveSpeed = navMeshAgent.speed; UpdateWeaponTypeParameter(); }
    ...
    void Update()
    {
        ...
        {
            if (navMeshAgent != null && navMeshAgent.enabled && !navMeshAgent.isStopped) { navMeshAgent.isStopped = true; navMeshAgent.ResetPath(); }
            return;
        }
        ...
        if (abilityHolder != null && abilityHolder.ActiveBeam != null) { if (navMeshAgent.hasPath) navMeshAgent.ResetPath(); return; }
        ...
        if (isFacingLocked && (TargetObject == null || (TargetObject.GetComponent<Health>() != null && TargetObject.GetComponent<Health>().currentHealth <= 0))) { isFacingLocked = false; }
        if (UIInteractionState.IsUIBlockingInput || EventSystem.current.IsPointerOverGameObject() || IsUIDragInProgress()) { return; }

        if (Input.GetKeyDown(KeyCode.Space) && Time.time >= nextDodgeTime) { StartCoroutine(DodgeRollCoroutine()); return; }
    ...
    void LateUpdate()
    {
        ...
        if (myHealth != null && myHealth.isDowned) return; // Stop Rotation
        HandleRotation();
        UpdateAnimator();
    }
    ...
    private IEnumerator MoveToInteract(GameObject targetObject) { if (targetObject == null) yield break; Vector3 destination = targetObject.transform.position; float currentInteractionRange = interactionRange; if (targetObject.TryGetComponent<DoorController>(out var door)) { if (door.interactionPoint != null) destination = door.interactionPoint.position; currentInteractionRange = door.activationDistance; } else if (targetObject.TryGetComponent<WorldMapExit>(out var exit)) { if (exit.interactionPoint != null) destination = exit.interactionPoint.position; currentInteractionRange = exit.activationDistance; } navMeshAgent.stoppingDistance = currentInteractionRange; navMeshAgent.SetDestination(destination); while (Vector3.Distance(transform.position, destination) > currentInteractionRange) { if (targetObject == null) { navMeshAgent.ResetPath(); yield break; } yield return null; } navMeshAgent.ResetPath(); if (targetObject.TryGetComponent<IInteractable>(out var interactable)) { interactable.Interact(this.gameObject); } else if (targetObject.TryGetComponent<DoorController>(out var doorComponent)) { doorComponent.UseDoor(); } interactionCoroutine = null; }
