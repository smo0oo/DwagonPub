...
public class PlayerStats : MonoBehaviour, ISerializationCallbackReceiver
{
    ...
    // --- NEW: Public accessor for Level (Fixes LootSlot error) ---
    ...
    public float RestoreMana(float amount) { float manaBeforeRestore = currentMana; currentMana += amount; if (currentMana > maxMana) currentMana = maxMana; OnManaChanged?.Invoke(); return currentMana - manaBeforeRestore; }
    public void CalculateFinalStats() { int baseStr = characterClass != null ? characterClass.strength : 0; int baseAgi = characterClass != null ? characterClass.agility : 0; int baseInt = characterClass != null ? characterClass.intelligence : 0; int baseFai = characterClass != null ? characterClass.faith : 0; int strengthBeforeGear = baseStr + bonusStrength; int agilityBeforeGear = baseAgi + bonusAgility; int intelligenceBeforeGear = baseInt + bonusIntelligence; int faithBeforeGear = baseFai + bonusFaith; finalStrength = strengthBeforeGear; finalAgility = agilityBeforeGear; finalIntelligence = intelligenceBeforeGear; finalFaith = faithBeforeGear; ApplyEquipmentPrimaryStats(); CalculateSecondaryRatings(strengthBeforeGear, agilityBeforeGear, intelligenceBeforeGear, faithBeforeGear); ConvertRatingsToPercentages(); UpdateResources(); GenerateTooltips(strengthBeforeGear, agilityBeforeGear, intelligenceBeforeGear, faithBeforeGear); OnStatsChanged?.Invoke(); OnManaChanged?.Invoke(); }
    ...
    private void UpdateResources() { if (playerHealth != null) { playerHealth.UpdateMaxHealth(100 + (finalStrength * 10)); } maxMana = 50 + (finalFaith * 10); if (currentMana > maxMana) currentMana = maxMana; }
    private void GenerateTooltips(int strBeforeGear, int agiBeforeGear, int intBeforeGear, int faiBeforeGear) { int playerLevel = partyManager != null ? partyManager.partyLevel : 1; float ratingDivisor = (partyManager != null ? partyManager.ratingToPercentDivisor : 50f) * playerLevel; if (ratingDivisor <= 0) ratingDivisor = 1; float strRate = partyManager != null ? partyManager.strengthConversionRate : 1f; float agiRate = partyManager != null ? partyManager.agilityConversionRate : 1f; float intRate = partyManager != null ? partyManager.intelligenceConversionRate : 1f; var sb = new StringBuilder(); string conversionText = $"Conversion: {ratingDivisor:F0} rating = 1%"; int bonusCritFromGear = secondaryStats.critRating - (int)(finalAgility * agiRate); sb.Clear().AppendLine($"{secondaryStats.critChance:F2}% Critical Strike Chance").AppendLine("--------------------").AppendLine($"From Agility: {(int)(agiBeforeGear * agiRate)} rating").AppendLine($"From Gear (Primary): {(int)((finalAgility - agiBeforeGear) * agiRate)} rating").AppendLine($"From Gear (Secondary): {bonusCritFromGear} rating").AppendLine("--------------------").AppendLine($"Total Rating: {secondaryStats.critRating}").Append(conversionText); secondaryStats.critChanceTooltip = sb.ToString(); sb.Clear().AppendLine($"{secondaryStats.spellCritChance:F2}% Spell Critical Chance").AppendLine("--------------------").AppendLine($"From Intelligence: {(int)(intBeforeGear * intRate)} rating").AppendLine($"From Gear (Primary): {(int)((finalIntelligence - intBeforeGear) * intRate)} rating").AppendLine("--------------------").AppendLine($"Total Rating: {secondaryStats.spellCritRating}").Append(conversionText); secondaryStats.spellCritChanceTooltip = sb.ToString(); int bonusDodgeFromGear = secondaryStats.dodgeRating - (int)(finalAgility * agiRate); sb.Clear().AppendLine($"{secondaryStats.dodgeChance:F2}% Dodge Chance").AppendLine("--------------------").AppendLine($"From Agility: {(int)(agiBeforeGear * agiRate)} rating").AppendLine($"From Gear (Primary): {(int)((finalAgility - agiBeforeGear) * agiRate)} rating").AppendLine($"From Gear (Secondary): {bonusDodgeFromGear} rating").AppendLine("--------------------").AppendLine($"Total Rating: {secondaryStats.dodgeRating}").Append(conversionText); secondaryStats.dodgeChanceTooltip = sb.ToString(); int bonusParryFromGear = secondaryStats.parryRating - (int)(finalStrength * strRate); sb.Clear().AppendLine($"{secondaryStats.parryChance:F2}% Parry Chance").AppendLine("--------------------").AppendLine($"From Strength: {(int)(strBeforeGear * strRate)} rating").AppendLine($"From Gear (Primary): {(int)((finalStrength - strBeforeGear) * strRate)} rating").AppendLine($"From Gear (Secondary): {bonusParryFromGear} rating").AppendLine("--------------------").AppendLine($"Total Rating: {secondaryStats.parryRating}").Append(conversionText); secondaryStats.parryChanceTooltip = sb.ToString(); int bonusBlockFromGear = secondaryStats.blockRating - (int)(finalStrength * strRate); sb.Clear().AppendLine($"{secondaryStats.blockChance:F2}% Block Chance").AppendLine("--------------------").AppendLine($"From Strength: {(int)(strBeforeGear * strRate)} rating").AppendLine($"From Gear (Primary): {(int)((finalStrength - strBeforeGear) * strRate)} rating").AppendLine($"From Gear (Secondary): {bonusBlockFromGear} rating").AppendLine("--------------------").AppendLine($"Total Rating: {secondaryStats.blockRating}").Append(conversionText); secondaryStats.blockChanceTooltip = sb.ToString(); int bonusHasteFromGear = secondaryStats.hasteRating - (int)(finalAgility * agiRate); sb.Clear().AppendLine($"{(secondaryStats.attackSpeed - 1.0f) * 100f:F2}% increased Attack Speed").AppendLine("--------------------").AppendLine($"From Agility: {(int)(agiBeforeGear * agiRate)} rating").AppendLine($"From Gear (Primary): {(int)((finalAgility - agiBeforeGear) * agiRate)} rating").AppendLine($"From Gear (Secondary): {bonusHasteFromGear} rating").AppendLine("--------------------").AppendLine($"Total Haste Rating: {secondaryStats.hasteRating}").Append(conversionText); secondaryStats.attackSpeedTooltip = sb.ToString(); sb.Clear().AppendLine($"{(1.0f - secondaryStats.cooldownReduction) * 100f:F2}% Cooldown Reduction").AppendLine("--------------------").AppendLine($"From Intelligence: {(int)(intBeforeGear * intRate)} rating").AppendLine($"From Gear (Primary): {(int)((finalIntelligence - intBeforeGear) * intRate)} rating").AppendLine("--------------------").AppendLine($"Total Rating: {secondaryStats.cooldownReductionRating}").Append(conversionText); secondaryStats.cooldownReductionTooltip = sb.ToString(); sb.Clear().AppendLine($"{secondaryStats.magicResistance:F2}% Magic Resistance").AppendLine("--------------------").AppendLine($"Total Rating: {secondaryStats.magicResistanceRating}").Append(conversionText); secondaryStats.magicResistTooltip = sb.ToString(); sb.Clear().AppendLine($"{secondaryStats.healingBonus:F2}% Healing Bonus").AppendLine("--------------------").AppendLine($"Total Healing Rating: {secondaryStats.healingBonusRating}").Append("Conversion: 2 rating = 1% bonus"); secondaryStats.healingBonusTooltip = sb.ToString(); if (playerHealth != null) { sb.Clear().AppendLine($"<color=green>Health: {playerHealth.maxHealth}</color>").AppendLine("--------------------").AppendLine("Base Health: 100").AppendLine($"From Strength: +{finalStrength * 10}"); secondaryStats.healthTooltip = sb.ToString(); } sb.Clear().AppendLine($"<color=blue>Mana: {maxMana}</color>").AppendLine("--------------------").AppendLine("Base Mana: 50").AppendLine($"From Faith: +{finalFaith * 10}"); secondaryStats.manaTooltip = sb.ToString(); }
}